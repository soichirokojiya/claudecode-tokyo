...
「毎回同じパターンのコンポーネント生成を指示するのが手間」「チーム内でコード品質にバラつきが出る」と感じていませんか？カスタムスキルを使えば、定型作業をスラッシュコマンドひとつで高品質に自動化できます。この記事ではSKILL.mdの書き方から実践例まで完全解説します。

## カスタムスキルとは？スラッシュコマンドで定型作業を自動化
...
## CLAUDE.mdとカスタムスキルはどう使い分ける？

カスタムスキルは特定タスクの手順書、CLAUDE.mdはプロジェクト全体のルールという位置づけです。CLAUDE.mdの詳しい書き方は[CLAUDE.mdの書き方ガイド](/articles/claude-md-guide)を参照してください。
...
- **カスタムスキルは`.claude/skills/`にSKILL.mdを置くだけ**で作成でき、スラッシュコマンドで呼び出せる
- **YAML フロントマター + マークダウン本文**の構成で、入力パラメータの定義も可能
- **具体的な指示（ファイル配置・命名規約・テスト要件）を書くほど出力品質が向上**する
- **Gitリポジトリに含めてチームで共有**すれば、コード品質をプロジェクト全体で統一できる

まずは最も頻繁に行う作業から1つスキルを作成してみましょう。さらにワークフローを効率化したい方は[Hooksの使い方ガイド](/articles/hooks-guide)や[生産性を3倍にするワークフロー10選](/articles/productivity-workflows)もあわせてお読みください。
